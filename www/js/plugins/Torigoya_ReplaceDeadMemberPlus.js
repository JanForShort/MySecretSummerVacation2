 ...
 * @default 0
 *
 * @param Changed Common Event
 * @type common_event
 * @desc 入れ替わりの発生時、誰に入れ替わるか確定したタイミングでコモンイベントを実行します。0で何も実行しません。入れ替わる控えがいない場合も実行しません。
 *
 * @param Changed Actor Id Variable
 * @type variable
 * @desc 入れ替わった控えのアクターのIDを代入する変数を指定します。0で何も代入しません。
 *
 * @param Before Changed Actor Id Variable
 * @type variable
 * @desc 入れ替わる前の、戦闘不能になったアクターのIDを代入する変数を指定します。0で何も代入しません。
 *
 * @param ■ こだわり設定
 *
...
(function (global) {
    ...
    ReplaceDeadMember.settings = (function () {
        ...
        return {
            ...
            changedCommonEvent: Number(parameters['Changed Common Event'] || 0),
            changedActorIdVariable: Number(parameters['Changed Actor Id Variable'] || 0),
            beforeActorIdVariable: Number(parameters['Before Changed Actor Id Variable'] || 0),
    ...
    ReplaceDeadMember.swapDeadMember = function () {
        ...
        for (var i = ReplaceDeadMember.getPartyStartIndex(); i < $gameParty.maxBattleMembers(); ++i) {
            ...
            for (var j = $gameParty.maxBattleMembers(); j < allMembers.length; ++j) {
                ...
                Array.prototype.push.apply(eventList, ReplaceDeadMember.setupCommonEvent(allMembers[j], changedActor));
                break;
            }
        }
        if (eventList.length) {
            $gameTroop._interpreter.setup(eventList, null);
        }
        return changed;
    };

    ReplaceDeadMember.setupCommonEvent = function (beforeBattleActor, changedBattleActor) {
        const eventList = [];
        if (ReplaceDeadMember.settings.changedActorIdVariable) {
            const TARGET_VARIABLE = ReplaceDeadMember.settings.changedActorIdVariable;
            eventList.push({
                code: 122,
                parameters: [TARGET_VARIABLE, TARGET_VARIABLE, 0, 0, changedBattleActor.actor().id]
            });
        }
        if (ReplaceDeadMember.settings.beforeActorIdVariable) {
            const TARGET_VARIABLE = ReplaceDeadMember.settings.beforeActorIdVariable;
            eventList.push({
                code: 122,
                parameters: [TARGET_VARIABLE, TARGET_VARIABLE, 0, 0, beforeBattleActor.actor().id]
            });
        }
        if (ReplaceDeadMember.settings.changedCommonEvent) {
            eventList.push({ code: 117, parameters: [ReplaceDeadMember.settings.changedCommonEvent] });
        }
        return eventList;
    }
    ...
    BattleManager.checkBattleEnd = function () {
        if (this._phase && this._phase != 'torigoya__replaceDeadMember__swap' && $gameParty.isAllDead() && ReplaceDeadMember.isReplaceable()) {
            this._phase = 'torigoya__replaceDeadMember__swap';
            onSwappedCallback = function () {
                this._phase = 'turn';
            }.bind(this);
        ...
        ReplaceDeadMember.swapDeadMember = function () {
            ...
            for (var i = ReplaceDeadMember.getPartyStartIndex(); i < $gameParty.maxBattleMembers(); ++i) {
                ...
                for (var j = 0; j < allMembers.length; ++j) {
                    ...
                    Array.prototype.push.apply(eventList, ReplaceDeadMember.setupCommonEvent(actor, changedActor));
